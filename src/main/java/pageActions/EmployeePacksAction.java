...
//	 }
	...
	public double getVehiclePriceForLender(String vehicleProgram, int days) throws Exception {
		utils.clickfield("xpath", calenderSoldDate);
		String pastOrfuturedate = calenderUtils.getCurrentDate(days, "MMM/dd/yyyy");
		calenderUtils.selectDate(pastOrfuturedate, "MMM/dd/yyyy");
		Thread.sleep(500);
		utils.inputfield("cssSelector", textbox, "100", 4);
		utils.inputfield("cssSelector", textbox, "5J6RW2H89NA004619", 5);
		utils.clickfield("xpath", getProducts);
	...
	public WebElement mileageSelectionInRateorConrtactPage(String termMonth) {
		WebElement termMonths = driver.findElement(By.xpath(
				"//td/div/span[contains(text(),'" + termMonth + "')]/../../../td/div[@class='cell cell--price']/span"));
		return termMonths;
	}

	public double getVehiclePriceForTermMonth(String vehicleProgram, String termMonth)
...
//		 String txt = utils.text("cssSelector", table, 0);
	...
	public double getVehiclePriceForTermMonth(String vehicleProgram, String termMonth, int days) throws Exception {
		utils.clickfield("xpath", calenderSoldDate);
		String pastOrfuturedate = calenderUtils.getCurrentDate(days, "MMM/dd/yyyy");
		calenderUtils.selectDate(pastOrfuturedate, "MMM/dd/yyyy");
		Thread.sleep(1000);
...
//		 String txt = utils.text("cssSelector", table, 0);
	...
	public double getVehiclePriceForTermMonthLender(String vehicleProgram, String termMonth)
			throws ParseException, InterruptedException {
		utils.inputfield("cssSelector", textbox, "Single", 0);
...
//		 String txt = utils.text("cssSelector", table, 0);
	...
	public double getVehiclePriceForTermMonthLender(String vehicleProgram, String termMonth, int days)
			throws Exception {
		utils.clickfield("xpath", calenderSoldDate);
		String pastOrfuturedate = calenderUtils.getCurrentDate(days, "MMM/dd/yyyy");
		calenderUtils.selectDate(pastOrfuturedate, "MMM/dd/yyyy");
...
//		 String txt = utils.text("cssSelector", table, 0);
	...
	public double getVehiclePrice(String vehicleProgram, int days) throws Exception {
//			 utils.inputfield("cssSelector", textbox, "Single", 0);
//			 //utils.getfield("cssSelector", "textbox").clear();
//			 utils.inputfield("cssSelector", textbox, "Test", 1);
		utils.clickfield("xpath", calenderSoldDate);
...
//		for(int i=1;i<=io.getRows().size();i++){
		if (allTableData.get(1).get("Role Type").equals(roleType)
				&& allTableData.get(1).get("Registration Status").equals(status)) {
...
//		Thread.sleep(2000);
	...
	public void editPack(int row) throws InterruptedException {
	HashMap<Integer, HashMap<String, WebElement>> allTableData = checkGridBodyDetails();
	Thread.sleep(2000);
	allTableData.get(row).get("Edit").click();
	}
}
